...
GSErrCode LibpartBuilder::PlaceLibpart(GS::Int32 libIndex)
{
	API_Element element;
	...
	GSErrCode err = ACAPI_Element_Create(&element, &elementMemo);
	ACAPI_DisposeElemMemoHdls(&elementMemo);

	return err;
}

static Int32 Do_CreateLibPart(const UnpackedElement& element, const std::string& /*baseGroupName*/, int runningNumber, IO::Location* loc)
{
	API_LibPart libPart{};
	GSErrCode err = NoError;
	...
	UInt32 rn = static_cast<UInt32>(runningNumber);
	GS::ucscpy(libPart.docu_UName, GS::UniString::SPrintf("Speckle Object %u", rn).ToUStr());
...
std::string LibpartBuilder::CreateLibPart(const UnpackedElement& element, const std::string& baseGroupName, int runningNumber)
{
	Int32 index = Do_CreateLibPart(element, baseGroupName, runningNumber, _location);
	if (index != 0)
	{
		libpartIndices.push_back(index);
	}

	return "asd";
}

std::string LibpartBuilder::CreateLibParts(const std::vector<UnpackedElement>& elements)
{
	int elemCount = 0;
