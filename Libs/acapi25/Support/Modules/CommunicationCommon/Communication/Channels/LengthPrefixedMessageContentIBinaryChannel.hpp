...
public:
	explicit LengthPrefixedMessageContentIBinaryChannel (const GS::IBinaryChannelRef& inputChannel,
		const GS::IProtocolX& inputProtocol = GS::GetNetworkByteOrderIProtocolX ());
	explicit LengthPrefixedMessageContentIBinaryChannel (GS::IBinaryChannel& inputChannel,
		const GS::IProtocolX& inputProtocol = GS::GetNetworkByteOrderIProtocolX ());
private:
			 LengthPrefixedMessageContentIBinaryChannel (const LengthPrefixedMessageContentIBinaryChannel&); // Disabled
public:
	virtual ~LengthPrefixedMessageContentIBinaryChannel ();

// Operator overloading:
private:
	const LengthPrefixedMessageContentIBinaryChannel&	operator = (const LengthPrefixedMessageContentIBinaryChannel&); // Disabled
