...
public:
	explicit LengthPrefixedMessageContentOBinaryChannel (const GS::OBinaryChannelRef& outputChannel,
		const GS::OProtocolX& outputProtocol = GS::GetNetworkByteOrderOProtocolX ());
	explicit LengthPrefixedMessageContentOBinaryChannel (GS::OBinaryChannel& outputChannel,
		const GS::OProtocolX& outputProtocol = GS::GetNetworkByteOrderOProtocolX ());
private:
			 LengthPrefixedMessageContentOBinaryChannel (const LengthPrefixedMessageContentOBinaryChannel&); // Disabled
public:
	virtual ~LengthPrefixedMessageContentOBinaryChannel ();

// Operator overloading:
private:
	const LengthPrefixedMessageContentOBinaryChannel&	operator = (const LengthPrefixedMessageContentOBinaryChannel&); // Disabled
