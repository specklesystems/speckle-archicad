...
public:
	...
	static CustomUnitVector2D Create (CoordinateType x, CoordinateType y, const DistanceTolerance& tol = DistanceTolerance (SmallEps))	CAN_THROW (UnitVectorCreationError);
	static CustomUnitVector2D Create (const Vector2<CoordinateType>& v, const DistanceTolerance& tol = DistanceTolerance (SmallEps))	CAN_THROW (UnitVectorCreationError);
