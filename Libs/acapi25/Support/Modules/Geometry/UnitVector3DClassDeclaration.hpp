...
public:
	...
	static CustomUnitVector3D Create (CoordinateType x, CoordinateType y, CoordinateType z, const DistanceTolerance& tol = DistanceTolerance (SmallEps))	CAN_THROW (UnitVectorCreationError);
	static CustomUnitVector3D Create (const Vector3<CoordinateType>& v, const DistanceTolerance& tol = DistanceTolerance (SmallEps))						CAN_THROW (UnitVectorCreationError);
