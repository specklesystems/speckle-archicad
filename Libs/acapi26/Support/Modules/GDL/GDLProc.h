...
typedef GSErr	GDL_CALL	SetBodyAttributesProc (GDL3DComponent component, GSUserData* userDataPtr, GSAttributeIndex bMat,
												   GSAttributeIndex ifill, PenType sbkgc, PenType sconc, PenType sfilc, GSAttributeIndex sltype);
...
struct GDL_DLL_EXPORT GDLMessageReporters {
	const GDLReportProcs		reportHandlers;
	const GDLMessageProcs		output;

	GDLMessageReporters ();
	GDLMessageReporters (const GDLReportProcs& reportHandlers, const GDLMessageProcs& output);

	GDLMessageReporters&	operator= (const GDLMessageReporters& other);
