...
CutParam FindCutParamOnSpecificEdge (const PolylineType& polyline, const UIndex edgeIndex, const Point2D& cutPointCoord)
{
		...
		for (typename PolylineType::ConstEdgeIterator edgeIt = polyLineToProcess.BeginEdge (); edgeIt != polyLineToProcess.EndEdge (); ++edgeIt) {
			...
			if (overlapCalculator.IsOverlap ()) {
				...
				if (Geometry::IsNear (begC, endC))
					continue;
				CutParam firstRemainderEnd = FindCutParamOnSpecificEdge (polyLineToProcess, edgeIndex, begC);
				CutParam lastRemainderBeg = FindCutParamOnSpecificEdge (polyLineToProcess, edgeIndex, endC);
