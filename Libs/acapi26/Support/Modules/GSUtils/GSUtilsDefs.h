...
#define		MEPSystem_Invalid				-1       // Invalid MEP
#define		OperationProfiles_Invalid		-1       // Invalid OperationProfile
...
#define ERRCHK_GUID(f, g) { GSErrCode errCode = (f); if (DBERROR_GUID (errCode != NoError, (g))) { return errCode; } }
#define ERRCHK_NO_ASSERT(f) { GSErrCode errCode = (f); if (errCode != NoError) { return errCode; } }
