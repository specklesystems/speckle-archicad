...
/** @brief Represents a `GS::Guid` in the API layer.
	...
	@struct API_Guid
	@remarks API_Guid is essentially a `GS::Guid`. It has the same size and structure, and only exists because of certain C++ language limitations: namely that a union (like @ref API_Element) cannot contain a class which has a constructor or destructor. Since @ref API_Elem_Head needs to contain a Guid member, and it cannot be a `GS::Guid`, the @ref API_Guid structure was introduced.
		...
		inline const GS::Guid APIGuid2GSGuid (const @ref API_Guid guid)
		inline @ref API_Guid GSGuid2APIGuid (GS::Guid guid)
		inline const @ref API_Guid GSGuid2APIGuid (const GS::Guid guid)
		...
		There's also a macro called APINULLGuid for the @ref API_Guid respresentation of the GS::NULLGuid constant.
...
/// @return Casting the API_Guid to a GS::Guid.
/// @ingroup Conversion
inline GS::Guid& APIGuid2GSGuid (API_Guid& guid)
{
...
/// @return A cast from API_Guid to GS::Guid.
/// @ingroup Conversion
...
/// @return A cast from GS::Guid to API_Guid.
/// @ingroup Conversion
...
/// @return A cast from a string to an API_Guid.
/// @ingroup Conversion
...
/// @return Converting the API_Guid to a GS::UniString.
/// @ingroup Conversion
