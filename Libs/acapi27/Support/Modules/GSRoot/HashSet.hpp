...
public:
	HashSetBase ()
	{
		...
		static_assert (GS::HasOperatorEquals<Type>,    "'Type' has no operator== ().");
	}
};


template <class Type>
class HashSet : public HashSetBase<Type> {
private:
	struct HashEntry {
...
public:
   ...
   ~HashSet ();
	...
	const Type&				GetTheOnlyItem () const;
	...
	Type&					ModifyValue	  (const Type& item);

	HashSet					Select (const std::function<bool (const Type&)>& condition) const;

	template <class ResultType>
	HashSet<ResultType>		Transform (const std::function<ResultType (const Type&)>& transformation) const;
...
template <class Type>
void	HashSet<Type>::DeleteAll (const std::function<bool (const Type&)>& condition)
{
...
template <class Type>
HashSet<Type>	HashSet<Type>::Select (const std::function<bool (const Type&)>& condition) const
{
	HashSet<Type> result;
	for (const Type& item : *this)
		if (condition (item))
...
template <class Type>
template <class ResultType>
HashSet<ResultType>	HashSet<Type>::Transform (const std::function<ResultType (const Type&)>& transformation) const
{
...
void	HashSet<Type>::Print (OChannel& oc) const
{
	...
	for (ConstIterator it = Enumerate (); it != nullptr; ++it) {
		char index[32];
		::sprintf (index, "[%04lu]  ", (ULongForStdio) i);
		oc << index;
...
template <class Type>
GSErrCode	Print (OChannel& oc, const HashSet<Type>& obj)
{
	obj.Print (oc);
