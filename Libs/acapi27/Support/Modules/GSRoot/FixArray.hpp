...
public:
	...
	constexpr const Type&	operator[] (UIndex idx) const;

	Type&					Get (UIndex idx);
	constexpr const Type&	Get (UIndex idx) const;

	Type&					GetFirst (void);
	constexpr const Type&	GetFirst (void) const;

	Type&					GetLast (void);
	constexpr const Type&	GetLast (void) const;
	...
	constexpr auto&					AsConst (void) const;
	...
	constexpr const Type*	GetContent (void) const;
...
template <class Type, USize Size>
bool	FixArray<Type, Size>::ContainsOnly (const std::function<bool (const Type&)>& condition) const
{
...
template <class Type, USize Size>
GSErrCode	Print (OChannel& oc, const FixArray<Type, Size>& obj)
{
	obj.Print (oc);
